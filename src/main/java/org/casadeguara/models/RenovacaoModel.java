...
/**
 * Esta classe é responsável por renovar um empréstimo de acordo
...
public class RenovacaoModel {
    ...
    public int renovar(List<Emprestimo> emprestimos) {
        ...
        try (Connection con = Conexao.abrir();
             CallableStatement cs = con.prepareCall("{call renovar(?,?,?)}")) {
    ...
    private Map<Integer, Integer> consultarRenovacoes(List<Emprestimo> emprestimos) {
        ...
        List<Integer> idemprestimos = emprestimos.stream()
                .map(Emprestimo::getIdEmprestimo)
    ...
    private boolean validarRenovacao(String titulo, int quantidadeRenovacoes) {
        return obedeceLimiteRenovacoes(quantidadeRenovacoes) || 
               permissaoParaProsseguir(titulo, quantidadeRenovacoes);
    }
