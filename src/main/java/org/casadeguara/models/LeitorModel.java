	...
	public ObservableList<Leitor> consultar(String nome, int resultados) {
		...
		try (Connection con = Conexao.abrir(); PreparedStatement ps = con.prepareStatement(query.toString())) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					leitores.add(new LeitorBuilder(rs.getString(2)).id(rs.getInt(1)).email(rs.getString(3))
							.telefone1(rs.getString(4)).telefone2(rs.getString(5)).logradouro(rs.getString(6))
							.bairro(rs.getString(7)).complemento(rs.getString(8)).cep(rs.getString(9))
							.cidade(rs.getString(10)).sexo(rs.getString(11)).rg(rs.getString(12)).cpf(rs.getString(13))
							.isAtivo(rs.getBoolean(14)).isTrabalhador(rs.getBoolean(15)).isIncompleto(rs.getBoolean(16))
							.build());
				}
			}
		} catch (SQLException ex) {
