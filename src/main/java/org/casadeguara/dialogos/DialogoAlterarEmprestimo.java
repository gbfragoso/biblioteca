...
public class DialogoAlterarEmprestimo extends Dialog<List<Integer>>{
    ...
    private CustomComboBox<Item> cbbExemplar;

    public DialogoAlterarEmprestimo(ObservableList<Item> items) {
        ...
        dialogPane.setContent(createContent(items));
        
        setResultConverter(button -> {
            List<Integer> dados = new ArrayList<>();
            if(button == ButtonType.OK) {
                int idemprestimo = Integer.parseInt(txtEmprestimo.getPlainText());
                int idexemplar = cbbExemplar.getSelectionModel().getSelectedItem().getId();
